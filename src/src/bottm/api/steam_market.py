...
class SteamMarketAPI:
    ...
    async def get_buy_orders(self, market_hash_name: str, max_retries: int = 3) -> BuyOrderInfo:
        ...
        if item_nameid is None:
            ...
            logger.info(f"Falling back to Steam beta orderbook for {market_hash_name} ({reason})")
            ...
            return BuyOrderInfo(success=False, error=f"nameid_not_found:{reason};{orderbook_info.error}")
