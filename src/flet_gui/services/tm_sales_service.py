...
class TmSalesService:
    ...
    async def _init_account_state(self, account_name: str):
        ...
        try:
            ...
            # ВАЖНО: Всегда реинициализируем Steam клиент для текущего event loop
            # Это необходимо, т.к. aiohttp сессии не могут переиспользоваться между разными event loops
            self.state.add_log(f"[INFO] [{account_name}] Reinitializing Steam client for current event loop...")
            try:
    ...
    async def _check_and_list_items(self):
        ...
        try:
            # Получаем предметы с истекшим холдом
            from src.database import TradesDatabase
            db = TradesDatabase()
            ...
            if not items:
                logger.debug("Hold check: no items ready for sale")
                return

            logger.info(f"Found {len(items)} items ready for sale")
            self.state.add_log(f"[INFO] Found {len(items)} items ready for sale")

            # Группируем предметы по аккаунтам
            items_by_account = {}
            for item in items:
            ...
            # Обновляем инвентарь для каждого аккаунта один раз
            for account_name, acc_items in items_by_account.items():
                acc_state = self._account_states.get(account_name)
                if acc_state and acc_state.csgotm_client and acc_state.status == SalesStatus.ONLINE:
                    self.state.add_log(f"[INFO] [{account_name}] Updating inventory once for {len(acc_items)} items...")
                    acc_state.csgotm_client.update_inventory()
                    # Ждем завершения обновления
                    await self._wait_for_inventory_update(account_name, max_wait_time=90)

            # Теперь выставляем все предметы
            for item in items:
                await self._list_item_for_sale(item, skip_inventory_update=True)
                await asyncio.sleep(2)  # Rate limiting

        except Exception as e:
            logger.error(f"Error checking items for sale: {e}")
        ...
        try:
            ...
            if not steam_id:
                ...
                self.state.add_log(
                    ...
                    "Item may still be on hold or already traded away."
                )
                return
            ...
            if not skip_inventory_update:
                ...
                # Ждем завершения обновления
                update_success = await self._wait_for_inventory_update(account_name, max_wait_time=60)
    ...
    async def _check_and_confirm_trades(self, account_name: str):
        ...
        try:
            ...
            for trade in trades:
                trade_id = trade.get('trade_id') or trade.get('tradeofferid')

                if trade_id:
                    success = await account.steam_client.accept_trade_offer(str(trade_id))

                    if success:
                        acc_state.stats.trades_confirmed += 1
                        self.state.add_log(f"[SUCCESS] [{account_name}] Trade {trade_id} confirmed")
                    else:
                        self.state.add_log(f"[ERROR] [{account_name}] Failed to confirm trade {trade_id}")
                ...
                try:
                    ...
                    if price_diff_pct >= threshold:  # Наша цена превышает порог
                        ...
                        logger.info(
                            ...
                            f"(our: {our_price:.0f}₽, top-1: {market_min_price:.0f}₽, diff: +{price_diff_pct:.1f}%)"
                        )
    ...
    async def _send_overpriced_notification(self, account_name: str, items: list):
        ...
        try:
            ...
            for idx, item in enumerate(items[:5], 1):  # Показываем только первые 5
                ...
                message += f"   Наша: {our_price:.0f}₽ | Рынок: {market_price:.0f}₽ (+{diff:.1f}%)\n"
    ...
    async def _wait_for_inventory_update(self, account_name: str, max_wait_time: int = 60) -> bool:
        ...
        while waited < max_wait_time:
            ...
            if status and not status.get("is_updating", False):
