...
class CsgoTmClient:
    ...
    def __init__(self, api_key: str, session: Optional[requests.Session] = None):
        ...
        if api_key:
            logger.info(f"CSGO.TM client initialized with API key: {api_key[:8]}...{api_key[-4:]} (length: {len(api_key)})")
        else:
    ...
    def update_price(self, item_id: str, new_price: float, currency: str = "RUB") -> bool:
        ...
        try:
            ...
            self._request("set-price", params={
                "item_id": item_id,
                "price": price_cents,
                "cur": currency
            })
    ...
    def get_trades_to_give(self) -> list[dict]:
        """
        Получить список трейдов на отдачу (проданные предметы).
        ...
        try:
            ...
            logger.info(f"Found {len(trades)} trades to give")
            return trades
        except CsgoTmError as e:
            error_msg = str(e)
            # "nothing" - это нормально, просто нет трейдов
            if "nothing" in error_msg.lower():
                logger.debug(f"No trades to give (API returned 'nothing')")
                return []
