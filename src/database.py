...
class TradesDatabase:
    ...
    def get_order_by_item_name(self, item_name: str, account_name: Optional[str] = None) -> Optional[dict]:
        """Get active order by item name, optionally filtered by account."""
        with self._get_connection() as conn:
            cursor = conn.cursor()
            if account_name:
                cursor.execute("""
                    SELECT * FROM active_orders
                    WHERE item_name = ? AND account_name = ? AND status = 'active'
    ...
    def get_active_profitable_items(self, min_profit: Optional[float] = None, limit: int = 100) -> list[dict]:
        ...
        with self._get_connection() as conn:
            ...
            if min_profit is not None:
                # Filter by recommended_profit_pct if available, otherwise fall back to profit_pct
                cursor.execute("""
                    SELECT * FROM profitable_items
                    WHERE is_active = 1
                      AND COALESCE(recommended_profit_pct, profit_pct) IS NOT NULL
                      AND COALESCE(recommended_profit_pct, profit_pct) >= ?
                    ORDER BY COALESCE(recommended_profit_pct, profit_pct) DESC
                    LIMIT ?
                """, (min_profit, limit))
            else:
                ...
                cursor.execute("""
                    ...
                    WHERE is_active = 1
                    ORDER BY
                        CASE WHEN COALESCE(recommended_profit_pct, profit_pct) IS NULL THEN 1 ELSE 0 END,
                        COALESCE(recommended_profit_pct, profit_pct) DESC
                    LIMIT ?
                """, (limit,))
