...
| CPU | 2 vCPU | сборка фронта в 1 vCPU упирается в память |
| RAM | **4 GB** | `next build` на 2 ГБ падает по OOM |
| Storage | 30 GB SSD | логи + node_modules |
...
`npm ci` требует `package-lock.json` — он в репозитории есть. Если сборка упала
по памяти, добавьте своп:
